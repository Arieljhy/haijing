function myTable(){
	...
	this.preobj;
	this.sec=function(obj){
		
		if(this.preobj!=null){
			var child1=this.preobj.childNodes;
		...
		for(var j1=0;j1<child.length;j1++){
			child[j1].style.backgroundColor="rgba(249,194,111,0.5)";
		}
	}
	this.creatTable=function(result,a,b,id) {
		var list = eval(result);//result为上面返回的json字符串
		$("#"+id).empty();
		var html = "";
		html = html + "<thead class='cf'><tr>";
		for(var j=0;j<b.length;j++){
			html = html + "<th class='numeric' style='text-align:center;'>"+b[j]+"</th>";  
		}
		...
		for(var obj in list){
			this.total++;
			html = html + "<tr id='my"+id+this.total+"'>"; 
			for(var i=0;i<a.length;i++){
				...
				html = html + "<td style='word-wrap: break-word;'  class='numeric' data-title="+b[i]+">"+str+"</td>";
			}
			html = html + "</tr>";
		} 
		html = html + "</tbody>";  
		$("#"+id).append(html);
		var aa=this;
		for(var z=1;z<=this.total;z++){
			$('#my'+id+z).on('click','',aa,function(){
				aa.sec(this);
			});
		}
	}
...
//					console.log(text);
					html = html + "<td class='numeric' data-title="+title[i]+">"+text+"</td>";   
				}
			}
			html = html + "</tr>";
		} 
