...
function drawMyTable(data){
	var params={
		titles:['名称','分类','地区','类型','年代','总集数','播放量'],
		dataTitles:['name','categoryStr','videoRegionStr','typeStr','videoYearStr','episodes','playCount'],
		data:data.data,
		total:data.total,
		index:data.index,
		reffunc:refresh,
	...
	MyTable.addOperation('table',{
		...
		operations:[/*{
			...
			title:'修改',
			func:function(value){
				window.location.href='videoInfo.html?type=upd&id='+value.id;
			}
		},{
			title:'删除',
			func:function(value){
